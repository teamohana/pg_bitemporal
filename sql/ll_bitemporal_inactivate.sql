CREATE OR REPLACE FUNCTION bitemporal_internal.ll_bitemporal_inactivate (p_schema_name text, p_table_name text, p_search_fields text, -- search fields
...
BEGIN
    ...
    EXECUTE format($u$ WITH updt AS ( UPDATE
        ...
        SELECT
            array_agg(% s)
            FROM updt $u$, v_table, p_asserted, p_search_fields, p_search_values, p_effective, p_effective, p_effective, p_asserted, p_asserted, v_serial_key, v_serial_key) INTO v_keys_old;
