CREATE OR REPLACE FUNCTION bitemporal_internal.ll_bitemporal_inactivate_select (p_schema_name text, p_table_name text, p_search_fields text, -- search fields
...
DECLARE
    ...
    -- so that we can reference this time v_keys int[];
    v_keys_old int[];
    ...
    EXECUTE format($u$ WITH updt AS ( UPDATE
            ...
            SET
                ...
                (temporal_relationships.is_overlaps (asserted, % L)
                    OR temporal_relationships.has_finishes (asserted, % L))
            RETURNING
                % s)
        SELECT
            array_agg(% s)
            FROM updt $u$, v_table, p_asserted, p_search_fields, p_values_selected_search, p_effective, p_effective, p_asserted, p_asserted, v_serial_key, v_serial_key) INTO v_keys_old;
