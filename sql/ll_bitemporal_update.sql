CREATE OR REPLACE FUNCTION bitemporal_internal.ll_bitemporal_update (p_schema_name text, p_table_name text, p_list_of_fields text, -- fields to update
...
BEGIN
    ...
    SELECT
        array_agg(% s)
        FROM updt $u$, v_table, p_asserted, p_search_fields, p_search_values, p_effective, p_effective, p_effective, v_serial_key, v_serial_key) INTO v_keys_old;
    ...
    EXECUTE format($i$ INSERT INTO % s (% s, effective, asserted)
        SELECT
            ...
            WHERE (% s) IN (% s) $i$, v_table, v_list_of_fields_to_insert_excl_effective, v_list_of_fields_to_insert_excl_effective, p_effective, p_asserted, v_table, v_serial_key, coalesce(array_to_string(v_keys_old, ','), 'NULL'));
    ...
    EXECUTE format($i$ WITH inst AS (INSERT INTO % s (% s, effective, asserted)
        ...
        SELECT
            array_agg(% s)
            FROM inst $i$, v_table, v_list_of_fields_to_insert_excl_effective, v_list_of_fields_to_insert_excl_effective, p_effective, p_asserted, v_table, v_serial_key, coalesce(array_to_string(v_keys_old, ','), 'NULL'), v_serial_key, v_serial_key) INTO v_keys;
    ...
    format($u$ UPDATE
        ...
        SET
            (% s) = (
                SELECT
                    % s)
            WHERE (% s) IN (% s) $u$, v_table, p_list_of_fields, p_list_of_values, v_serial_key, coalesce(array_to_string(v_keys, ','), 'NULL'));
